...
export const createUserValidation = [
  ...
  body("password")
    ...
    .isStrongPassword().withMessage("La contraseña debe tener mínimo 8 caracteres, incluyendo mayúsculas, minúsculas, números y símbolos."),
  body("role")
    .optional()
    .isIn(["user", "admin"]).withMessage("El rol especificado no es válido. Debe ser 'user' o 'admin'."),
  body("profile.firstName")
    .notEmpty().withMessage("El nombre del perfil es obligatorio.")
    .isString().withMessage("El nombre debe ser una cadena de texto.")
    .isLength({ min: 2, max: 50 }).withMessage("El nombre debe tener entre 2 y 50 caracteres."),
  body("profile.lastName")
    .notEmpty().withMessage("El apellido del perfil es obligatorio.")
    .isString().withMessage("El apellido debe ser una cadena de texto.")
    .isLength({ min: 2, max: 50 }).withMessage("El apellido debe tener entre 2 y 50 caracteres."),
  body("profile.biography")
    .optional()
    .isString().withMessage("La biografía debe ser una cadena de texto.")
...
export const updateUserValidation = [
  param("id")
    ...
    .isMongoId().withMessage("El id del tag no es un ObjectId válido.")
    .custom(async (value) => {
  ...
  body("password")
    ...
    .matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{8,}$/).withMessage("La nueva contraseña debe tener mínimo 8 caracteres, incluyendo mayúsculas, minúsculas, números y símbolos."),
  body("role")
    .optional()
    .isIn(["user", "admin"]).withMessage("El rol especificado no es válido."),
  body("profile.firstName")
    .optional()
    .isString().withMessage("El nombre debe ser una cadena de texto.")
    .isLength({ min: 2, max: 50 }).withMessage("El nombre debe tener entre 2 y 50 caracteres."),
